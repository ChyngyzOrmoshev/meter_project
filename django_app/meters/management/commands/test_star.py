...
class Command(BaseCommand):
    ...
    def _import_xml(self, file_path):
        """Парсит XML и импортирует показания в БД (в отдельном потоке)."""
        def import_task():
            ...
            try:
                ...
                # Загружаем все активные устройства
                ...
                for d in all_devices:
                    ...
                    # Определяем, является ли устройство Star Power
                    is_star = False
                    if d.askue_id and str(d.askue_id) == '14':
                        is_star = True
                    elif d.api_id and d.api_id.upper().startswith('ST'):
                        is_star = True
                    # Также можно проверить модель, если нужно
                    if is_star and len(d.serial_number) >= 8:
                        last8 = d.serial_number[-8:]
                        devices_by_last8[last8] = d  # если конфликт, последний перезапишет
                ...
                for mreading in root.findall('mreadings'):
                    ...
                    # Сначала ищем по полному номеру
                    device = devices_by_full.get(sn)
                    if not device and len(sn) >= 8:
                        # Если не найдено, пробуем по последним 8 цифрам (только для Star)
                        last8 = sn[-8:]
