    ...
    def sync(self):
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                # 5. Скачивание (проверенный метод из теста)
                ...
                # Ждём появления имени файла (.xml)
                page.wait_for_selector('span:has-text(".xml")', state='visible', timeout=180000)
                file_span = page.locator('span:has-text(".xml")').nth(-1)
                file_name = file_span.text_content()
                logger.info(f"✅ Export completed, file name: {file_name}")

                time.sleep(2)

                # Находим родительский контейнер (диалог)
                parent = file_span.locator('xpath=ancestor::div[contains(@class, "el-dialog") or contains(@class, "el-form-item") or contains(@class, "el-row")]').first
