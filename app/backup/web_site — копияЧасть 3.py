...
# ==============================================================================
# === РАЗДЕЛ 3: СБОР ПОКАЗАНИЙ (ФОРМЫ ВВОДА) ===
# ==============================================================================
elif page == "📉 Сбор показаний (Readings)":
    st.subheader("Ввод текущих показаний и история")

    # Переводим список в set. Поиск "if x in set" занимает O(1) времени,
    # в отличие от списка, который при поиске перебирается с начала до конца O(N).
                    ...
                    with st.form("bulk_reading_form", clear_on_submit=True):
                        ...
                        if st.form_submit_button("Сохранить группу показаний"):
                            ...
                            if lines:
                                ...
                                for line in lines:
                                    parts = line.split()
                                    # ИСПРАВЛЕНО: Проверяем структуру и берем конкретные индексы [0] и [1]
                                    if (
                                        len(parts) >= 2
                                        and parts[0] in all_serial_numbers_set
                                    ):
                                        try:
                                            # Заменяем запятую на точку, чтобы строка гарантированно стала float
                                            ...
                                            pass  # Игнорируем строки с ошибками в числах

                                if inserts:
                                    # Оптимизация: Пакетная вставка всех показаний одним махом
                                    ...
                                    st.error(
                                        "Не найдено корректных строк для добавления. Проверьте номера счетчиков и формат!"
                                    )
