...
all_models = sorted(list(models_col.distinct("model_name")))
all_serial_numbers = sorted(list(devices_col.distinct("serial_number")))
...
# ==============================================================================
    ...
    # Вывод таблицы устройств с фильтрами
    ...
    if devices:
        ...
        with f1:
            s_search = st.text_input("🔍 Поиск по заводскому номеру:")
        with f2:
        ...
        with f3:
            m_filter = st.selectbox(
                "Фильтр по модели:",
                ["Все"] + sorted(df_devices["model_name"].unique().tolist()),
            )

        if s_search:
            final_df = final_df[
                final_df["Заводской номер"]
                .astype(str)
                .str.contains(s_search, case=False)
            ]
        # ИСПРАВЛЕНО: Теперь проверяем s_filter вместо f2, счетчики отобразятся на экране!
        if s_filter != "Все":
            final_df = final_df[final_df["Статус прибора"] == s_filter]
        if m_filter != "Все":
            final_df = final_df[final_df["Модель"] == m_filter]
        st.dataframe(final_df, use_container_width=True, hide_index=True)
    else:
        st.info("В реестре приборов пока пусто.")
# ==============================================================================
