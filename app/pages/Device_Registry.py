...
def show():
    ...
    # ====== РАЗДЕЛ РЕДАКТИРОВАНИЯ ЗАВОДСКОГО НОМЕРА ======
    if user_role in ["admin", "operator"]:
        st.markdown("---")
        with st.expander("✏️ Редактировать заводской номер прибора"):
            st.info("Введите текущий заводской номер и новый номер, который нужно присвоить.")
            with st.form("edit_sn_form", clear_on_submit=True):
                old_sn = st.text_input("Текущий заводской номер:").strip()
                new_sn = st.text_input("Новый заводской номер:").strip()
                submit_edit = st.form_submit_button("Изменить номер")
                if submit_edit:
                    if not old_sn or not new_sn:
                        st.error("❌ Оба поля должны быть заполнены!")
                    elif old_sn == new_sn:
                        st.warning("⚠️ Новый номер совпадает с текущим. Изменение не требуется.")
                    else:
                        old_device = devices_col.find_one({"serial_number": old_sn})
                        if not old_device:
                            st.error(f"❌ Прибор с номером {old_sn} не найден в реестре!")
                        else:
                            if devices_col.find_one({"serial_number": new_sn}):
                                st.error(f"❌ Новый номер {new_sn} уже зарегистрирован в системе!")
                            else:
                                try:
                                    result = devices_col.update_one(
                                        {"serial_number": old_sn},
                                        {"$set": {"serial_number": new_sn}}
                                    )
                                    if result.modified_count > 0:
                                        st.success(f"✅ Заводской номер успешно изменён с {old_sn} на {new_sn}!")
                                        st.rerun()
                                    else:
                                        st.error("❌ Не удалось обновить номер.")
                                except DuplicateKeyError:
                                    st.error(f"❌ Новый номер {new_sn} уже существует (дубликат).")
                                except Exception as e:
